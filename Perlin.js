...
class Perlin{
	...
	perlin(x, y ,z){
		...
		let xi = parseInt(x) & 255;
		let yi = parseInt(y) & 255;
		let zi = parseInt(z) & 255;
	 ...
	 * @param {number} t The value we are applying the fade to
	 */
	fade(t){
	 ...
	 * @param {number} a The first value for the interpolation bound
	 * @param {number} b The second value for the interpolation bound
	 * @param {number} x The value we are interpolating
	 */
	lerp(a, b, x){
		return a + x * (b - a);
	}
